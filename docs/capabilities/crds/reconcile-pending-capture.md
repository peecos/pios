...
## Core meaning
- **Capability purpose:** Preserve and resolve submitted work whose canonical Core acceptance has not yet been confirmed.
...
- **Policies / invariants:** Pending work is not treated as accepted canonical state; potentially unique content is not silently discarded while durability is unresolved; acceptance is verified against canonical Core; retries are idempotent; owner/Core identity and grants are current; conflicts and duplicates remain visible.
...
|---|---|---|---|
| Captures without confirmed Core acceptance require an explicit accepted, retryable, failed, preserved, or authorized-discard disposition. | reasonable inference | proposed | [D02](../evidence/statement-provenance.md). |
